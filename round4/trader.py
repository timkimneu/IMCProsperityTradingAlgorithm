...
COCONUT_SPREAD = "COCONUT_SPREAD"
COUPON_SPREAD = "COUPON_SPREAD"
...
class Trader:
    ...
    def update_prices(self, state :TradingState) -> None:
        ...
        self.pred_last_price["COCONUT_DIFF"] = 15.4 * self.get_mid_price(COCONUT_COUPON, state)
    
    def update_spreads(self, state: TradingState) -> None:
        gift_spread = self.pred_last_price[SUM_OF_PARTS] - self.pred_last_price[GIFT_BASKET]
        coconut_spread = self.pred_last_price["COCONUT_DIFF"] - self.pred_last_price[COCONUT]

        # update the spread
        self.spread[SUM_GIFT_BASKET_SPREAD].append(gift_spread)
        self.spread[COCONUT_SPREAD].append(coconut_spread)
    ...
    def create_basket_order(self,state,buy:bool, volume:int, product) -> dict[PRODUCTS:List[Order]]:
        ...
        if buy:
            orders.append(Order(product, math.ceil(self.ema_prices[product]), (total_fill) * -sign)) # negative since theposition will be the inverse
        else:
            orders.append(Order(product, math.floor(self.ema_prices[product]), (total_fill) * -sign))

        results.update({product:orders})
        return results
         
    def gift_basket_sum_of_parts_pair_trade_naive(self, state: TradingState, product, in_spread) -> dict[PRODUCTS:List[Order]]:
        """
        """

        spread = self.spread[in_spread][-1]
        if len(self.spread[in_spread]) < self.window:
            raise ValueError(
                "Trading before end of window" + str(len(self.spread[in_spread])) + str(self.window))

        spread_30_days = self.spread[in_spread][-self.window:]
        ...
        sell = False
        last_signal = None
        if self.signals_history[in_spread]:
            last_signal = self.signals_history[in_spread][-1][1]
            last_timestep = self.signals_history[in_spread][-1][0]

        if z_score < -2.0 and not spread > -350 or (
                last_signal and last_signal == 1 and state.timestamp - last_timestep <= 200):
            ...
            # else:
            results.update(self.create_basket_order(state, True, 2 * SPREAD_VOL, product))
            ...
            # else:
            results.update(self.create_basket_order(state, False, 2 * SPREAD_VOL, product))

        if buy:
            self.signals_history[in_spread].append((state.timestamp, 1))
        elif sell:
            self.signals_history[in_spread].append((state.timestamp, -1))
    ...
    def gift_basket_sum_of_parts_pair_trade(self, state: TradingState, product, spread) -> dict[PRODUCTS,List[Order]]:
        """
        ...
        if self.gift_basket_sum_of_parts_strategy == STRATEGY_TYPE.NAIVE:
            order_dict = self.gift_basket_sum_of_parts_pair_trade_naive(state, product, spread)
        else:
            raise ValueError("Strategy not implemented")
        return order_dict
    
    def roses_gift_basket_pair_trade_naive(self, state: TradingState) -> dict[PRODUCTS,List[Order]]:
        return
        ...
        for product in state.order_depths:
            ...
            if product == GIFT_BASKET:
                if self.price_history[product] and len(self.price_history[product]) >= self.window:
                    # start the strategy
                    order_dict = self.gift_basket_sum_of_parts_pair_trade(state, product, SUM_GIFT_BASKET_SPREAD)
                    logger.print("Signals: " + str(self.signals_history[SUM_GIFT_BASKET_SPREAD]))
            elif product == COCONUT:
                if self.price_history[product] and len(self.price_history[product]) >= self.window:
                    order_dict = self.gift_basket_sum_of_parts_pair_trade(state, product, COCONUT_SPREAD)
